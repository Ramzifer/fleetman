...
sh "export MINIKUBE_HOME=/var/lib/jenkins/.minikube && minikube ssh 'cd /tmp && docker build -t fleetman-webapp: .'"
echo 'build complete'
...
sh "sed -i -r 's|richardchesterwood/k8s-fleetman-webapp:release2|fleetman-webapp:|' /var/lib/jenkins/k8s-fleetman-deploy/replicaset-webapp.yml"
